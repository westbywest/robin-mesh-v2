...
private_isolation=$(uci get iprules.filter.AP2_isolation)
...
config wifi-iface private
	...
	option network  ap2
	option mode     ap
	option isolate  ${private_isolation}
...
#prepare dhcp private pool
echo "${Myap_s_lease},${Myap_e_lease},255.255.255.0,2h" > /tmp/private_pool
#update UCI node
uci set node.general.IP_Myap="${ap2_ipaddr}/${MyAP_PREFIX}"
uci commit node
#
